...
import { read, close, unlink, closeSync, unlinkSync, open, write } from 'node:fs';
...
const GraphQLUpload = new GraphQLScalarType(uploadScalarConfig);
...
class FieldTooLargeError extends UploadError {
    constructor(fieldName, maxSize) {
        super(`The '${fieldName}' multipart field value exceeds the ${maxSize} byte size limit.`, UploadErrorCode.FIELD_TOO_LARGE, 413);
    }
}
...
class MissingOperationsError extends UploadError {
    constructor() {
        super(`Missing multipart field 'operations' (${GRAPHQL_MULTIPART_REQUEST_SPEC_URL}).`, UploadErrorCode.MISSING_OPERATIONS);
    }
}
...
class InvalidJSONError extends UploadError {
    constructor(fieldName) {
        super(`Invalid JSON in the '${fieldName}' multipart field (${GRAPHQL_MULTIPART_REQUEST_SPEC_URL}).`, UploadErrorCode.INVALID_JSON);
    }
}
...
export { DEFAULT_MAX_FIELD_SIZE, DEFAULT_MAX_FILES, DEFAULT_MAX_FILE_SIZE, FieldTooLargeError, FileMissingError, FileTooLargeError, GRAPHQL_MULTIPART_REQUEST_SPEC_URL$1 as GRAPHQL_MULTIPART_REQUEST_SPEC_URL, GraphQLUpload, InvalidJSONError, InvalidMapError, InvalidMultipartError, MissingMapError, MissingOperationsError, ReadAfterDestroyedError, ReadAfterReleasedError, ReadStream, RequestDisconnectedError, StreamError, TooManyFilesError, Upload, UploadError, UploadErrorCode, WriteStream, graphqlUploadExpress, graphqlUploadKoa, ignoreStream, processRequest, sanitizeFilename, validateFileExtension, validateMimeType, validateOptions };
